...
{% if not cart_page %}
  ...
  <div class="mb-2">
    ...
    <div class="js-visible-on-cart-filled h5 font-big row mb-1" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-subtotal">
    ...
    {% for promotion in cart.promotional_discount.promotions_applied %}
      {% if not promotion.is_subscription_promotion %}
        ...
        {% else %}
          {% set id = 'all' %}
        {% endif %}
          <span class="js-total-promotions-detail-row row mb-2" id="{{ id }}">
            <span class="col pr-3">
...
{% if cart_page %}
  ...
  <div id="cart-sticky-summary" class="position-sticky-md cart-page-totals">
    ...
    <div class="mb-2">
      <div class="js-visible-on-cart-filled h5 row no-gutters font-big mb-1" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-subtotal">
      ...
      {% for promotion in cart.promotional_discount.promotions_applied %}
        {% if not promotion.is_subscription_promotion %}
          ...
          {% else %}
            {% set id = 'all' %}
          {% endif %}
            <span class="js-total-promotions-detail-row row no-gutters mb-2" id="{{ id }}">
              <span class="col pr-3">
                {% if promotion.discount_script_type != "custom" %}
                  {% if promotion.discount_script_type == "NAtX%off" %}
                    {{ promotion.selected_threshold.discount_decimal_percentage * 100 }}% OFF
    ...
    {% if show_calculator_on_cart %}
      <div id="shipping-cost-container" class="js-fulfillment-info js-visible-on-cart-filled js-shipping-cost-table h6 font-body mb-2 row no-gutters" {% if cart.items_count == 0 or (not cart.has_shippable_products) %}style="display:none;"{% endif %}>
...
{% endif %}
    ...
    <div class="js-cart-total-container js-visible-on-cart-filled" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-total">
      <div class="h2 row {% if cart_page %}no-gutters{% endif %} font-huge mb-2">
        ...
        <span class="col-12 {% if cart_page %}pr-md-0{% endif %}">
          {{ component('payment-discount-price', {
              ...
              location: 'cart',
              container_classes: 'font-small text-right',
            }) 
          }}

          {% if not settings.payment_discount_price %}
            {{ component('installments', {'location': 'cart', 'short_wording' : true, container_classes: { installment: "font-small text-right"}}) }}
          {% endif %}
        </span>
      </div>
